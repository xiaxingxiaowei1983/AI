...
- 用户画像信息：收集用户的年龄、性别、文化背景、职业等基本信息，用于分析适配和报告风格选择
  ...
  - 使用场景：个人成长/心理咨询/教育评估（影响报告形式）
- 品牌与产品知识：
  - 见 [references/brand-product-guide.md](references/brand-product-guide.md)（品牌核心、品牌定位、产品介绍、电梯演讲、推广文案）
...
- 标准流程：
  ...
  6. **生成双份报告**
     - **报告1：专业分析报告**（Markdown 格式，仅文档）
       - 输出文件：`professional-analysis.md`
       - 内容：详细、系统、理论化的专业分析
       - 结构：包含完整的视觉特征识别、多维度心理分析、风险评估、专业建议
       - 面向：专业人士（心理咨询师、治疗师等）

     - **报告2：客户洞察报告**（HTML + 配图 + 长图文）
       - 输出文件：
         - `client-insight.html`（HTML 格式，包含智能配图）
         - `client-insight-image.png`（长图文格式，适合分享）
         - `illustrations/` 目录（包含4张智能配图）
       - 内容：温暖、友好、易于理解的核心洞察和行动建议
       - 结构：深度洞察（4段） + 行动建议（3条） + 最后的邀请
       - 配图：4张智能插图（封面、深度洞察、行动建议、最后的邀请）
       - 面向：普通用户（客户本人）

     **HTML 客户洞察报告的智能配图策略**：
     - **配图风格**：统一使用 **warm** 风格（温暖、友好、人性化）
     - **配图数量**：4张
     - **配图位置与主题**：
       1. 封面插图（报告开头）：整体心理状态的视觉隐喻
       2. 深度洞察插图（深度洞察部分）：关键意象的可视化表达
       3. 行动建议插图（行动建议部分）：积极行动的视觉引导
       4. 最后的邀请插图（最后的邀请部分）：温暖支持的视觉表达
     - **warm 风格特征**：
     ...
     **智能配图流程**：
     ...
     2. **生成配图提示词**：基于报告内容、warm 风格特征、情感基调生成详细的图像生成提示
     3. **生成插图**：使用智能体的图像生成能力，根据提示词生成4张插图
     4. **整合到 HTML**：将生成的插图插入到 HTML 报告的相应位置
     5. **生成长图文**：将 HTML 报告转换为长图文格式（PNG）
...
## 资源索引
- 品牌与产品：见 [references/brand-product-guide.md](references/brand-product-guide.md)（品牌核心、品牌定位、产品介绍、电梯演讲、推广文案）
...
- 智能体工作流：见 [references/agent-workflow-prompt.md](references/agent-workflow-prompt.md)（完整的八步工作流和配图生成策略）
...
- 报告输出：专业分析报告（Markdown）+ 客户洞察报告（HTML + warm 配图 + 长图文）
- 风格选择：统一使用 warm 风格，温暖、友好、适合儿童
