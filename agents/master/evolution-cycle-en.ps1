# Auto Evolution System Full Cycle Execution Script
# Current Time: $(Get-Date -Format "yyyy-MM-dd HH:mm:ss")

# Set NODE_PATH to include custom skills directory
$env:NODE_PATH = (Resolve-Path "~/.openclaw/skills/custom-skills/").Path

Write-Host "=== Starting Auto Evolution System Full Cycle ===" -ForegroundColor Green
...
try {
    $evalResult = openclaw agent --agent main --message "调用agent-optimizer的evaluate动作，参数：agent=main，metrics=all，depth=full" 2>&1
...
try {
    $capOptResult = openclaw agent --agent main --message "调用capability-optimizer的optimize动作，参数：agent=main，focus=all，tokenReduction=true，efficiencyBoost=true" 2>&1
...
try {
    $nightResult = openclaw agent --agent main --message "调用night-evolution的execute动作，参数：mode=continuous，duration=auto，strategy=balanced" 2>&1
...
try {
    $reportResult = openclaw agent --agent master --message "调用evolution-monitor-plus的generateReport动作，参数：agent=master，period=full_cycle，format=text，includeRecommendations=true" 2>&1
...
"=== Auto Evolution System Full Cycle Log ===" | Out-File -FilePath $logPath -Encoding utf8
