...
async function copyHtmlFromBrowser(cdp: CdpConnection, htmlFilePath: string): Promise<void> {
  ...
  // 构建跨平台兼容的文件URL
  let fileUrl = `file://${absolutePath}`;
  // 修复Windows路径分隔符和驱动器号格式
  if (process.platform === 'win32') {
    fileUrl = fileUrl.replace(/\\/g, '/');
    // 修复Windows驱动器号格式 (file://C:/path -> file:///C:/path)
    fileUrl = fileUrl.replace(/file:\/\/(\w):\//i, 'file:///$1:/');
  }
    ...
    if (process.platform === 'darwin') {
      ...
      for (const tool of tools) {
        try {
          ...
          console.log(`[wechat] Tool ${tool} not available: ${e}`);
          continue;
        }
      }
      console.log('[wechat] No Linux paste tools available');
    } else if (process.platform === 'win32') {
      try {
        const ps = `Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait("^v")`;
        const result = spawnSync('powershell.exe', ['-NoProfile', '-Command', ps], { stdio: 'ignore' });
        if (result.status === 0) {
          await sleep(1000);
          console.log('[wechat] Paste command sent via PowerShell');
          return;
        } else {
          console.log('[wechat] PowerShell paste command failed');
        }
      } catch (e) {
        console.log('[wechat] PowerShell not available: ' + e);
      }
    } else {
      console.log(`[wechat] Platform ${process.platform} not supported for paste operations`);
    }
  } catch (error) {
    console.error(`[wechat] Paste failed: ${error}`);
  }
  
  console.log('[wechat] Paste operation completed (or no compatible method found)');
}
...
if (import.meta.url === `file://${process.argv[1]}` || import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  // Only run main when executed directly, not the test code
